...
public class RecursionAndDynamic {
     ...
     *        
     * Performance: O(n*2^n) -- an iterative implementation would bring a slight optimisation
     * 
     ...
     * to generate our new subsets, hence we get n*2^n runtime.
     * 
     */
    ...
    private static ArrayList<ArrayList<Integer>> getSubsets(ArrayList<Integer> set, int index) {
        ...
        if(set.size() == index) {
            ...
            for(ArrayList<Integer> subset: allSubsets) {
                // Create a subset that consists of the previous subset + this item
            ...
            // Join previous subsets with the subsets for this item
    ...
    /*
     * Generates all valid permutations for n pairs of parentheses.
     * 
     * This can be done in a similar way to generating string
     * permutations, but that results in lots of duplicates being generated.
     * This can be mitigated by using a HashSet, but it still inefficient.
     * 
     * Point: If there are left parenthesis left,
     *        we can always insert a left parenthesis.
     *        
     *        As long as there are more left parenthesis
     *        than right parenthesis, we can insert a right parenthesis.
     *        (If you have less right than left, it means you inserted
     *         a left parenthesis before a right, which is invalid.)
     */
    public static ArrayList<String> getParenthesesPermutations(int n) {
        char [] permutation = new char[n*2];
        ArrayList<String> permutations = new ArrayList<String>();
        
        getParenthesesPermutations(permutations, n, n, permutation, 0);
        
        return permutations;
    }
    
    private static void getParenthesesPermutations(ArrayList<String> permutations, int leftRemaining, int rightRemaining, char[] permutation, int index) {
        ...
        if(leftRemaining == 0 && rightRemaining == 0) {
            permutations.add(String.copyValueOf(permutation));
            return;
        }
        
        permutation[index] = '(';
        getParenthesesPermutations(permutations, leftRemaining-1, rightRemaining, permutation, index+1);
